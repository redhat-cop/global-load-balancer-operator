...
var (
	...
	keyExists   = struct{}{}
	nonExistent redhatcopv1alpha1.ClusterReference
)
...
type Set struct {
	m map[redhatcopv1alpha1.ClusterReference]struct{}
}

// New creates and initializes a new Set.
func New(ts ...redhatcopv1alpha1.ClusterReference) *Set {
...
func NewWithSize(size int) *Set {
	return &Set{make(map[redhatcopv1alpha1.ClusterReference]struct{}, size)}
}

// Add includes the specified items (one or more) to the Set. The underlying
// Set s is modified. If passed nothing it silently returns.
func (s *Set) Add(items ...redhatcopv1alpha1.ClusterReference) {
...
func (s *Set) Remove(items ...redhatcopv1alpha1.ClusterReference) {
	for _, item := range items {
		delete(s.m, item)
	}
}
...
func (s *Set) Pop() redhatcopv1alpha1.ClusterReference {
	for item := range s.m {
		delete(s.m, item)
		return item
	}
...
// returned.
func (s *Set) Pop2() (redhatcopv1alpha1.ClusterReference, bool) {
	for item := range s.m {
		delete(s.m, item)
		return item, true
	}
...
func (s *Set) Has(items ...redhatcopv1alpha1.ClusterReference) bool {
	has := false
	for _, item := range items {
...
func (s *Set) HasAny(items ...redhatcopv1alpha1.ClusterReference) bool {
	has := false
	for _, item := range items {
		if _, has = s.m[item]; has {
			break
...
func (s *Set) Clear() {
	s.m = make(map[redhatcopv1alpha1.ClusterReference]struct{})
}
...
func (s *Set) IsEqual(t *Set) bool {
	...
	equal := true
	t.Each(func(item redhatcopv1alpha1.ClusterReference) bool {
		_, equal = s.m[item]
...
func (s *Set) Each(f func(item redhatcopv1alpha1.ClusterReference) bool) {
	for item := range s.m {
		if !f(item) {
			break
		}
...
func (s *Set) List() []redhatcopv1alpha1.ClusterReference {
	v := make([]redhatcopv1alpha1.ClusterReference, 0, s.Size())
	for item := range s.m {
		v = append(v, item)
	}
	return v

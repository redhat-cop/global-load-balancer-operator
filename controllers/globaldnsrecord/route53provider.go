...
func (r *GlobalDNSRecordReconciler) createRoute53Record(context context.Context, instance *redhatcopv1alpha1.GlobalDNSRecord, globalzone *redhatcopv1alpha1.GlobalDNSZone, endpointMap map[string]EndpointStatus) (reconcile.Result, error) {
...
func (r *GlobalDNSRecordReconciler) ensureRoute53HealthCheck(instance *redhatcopv1alpha1.GlobalDNSRecord, route53Client *route53.Route53, ip string) (string, error) {

	healthChecks, err := route53Client.ListHealthChecks(&route53.ListHealthChecksInput{})
	if err != nil {
		r.Log.Error(err, "unable to list route53 healthchecks")
		return "", err
	}
	...
	for _, healthCheck := range healthChecks.HealthChecks {
		if instance.Spec.HealthCheck.HTTPGet != nil && healthCheck.HealthCheckConfig.FullyQualifiedDomainName != nil && *healthCheck.HealthCheckConfig.FullyQualifiedDomainName == instance.Spec.HealthCheck.HTTPGet.Host && *healthCheck.HealthCheckConfig.IPAddress == ip {
			r.Log.V(1).Info("found existing", "health check for", *healthCheck.HealthCheckConfig.FullyQualifiedDomainName)
	...
	if currentHealthCheck == nil {
		...
		healthCheckID, err := r.createHealthCheck(instance.Spec.HealthCheck, route53Client, ip)
		if err != nil {
			r.Log.Error(err, "unable to create route53 healthcheck", "for probe", instance.Spec.HealthCheck)
			return "", err
		}
	...
	newHealthCheckConfig, err := r.getAWSHealthCheckConfig(instance.Spec.HealthCheck, ip)
	if err != nil {
		r.Log.Error(err, "unable to convert probe to aws health check", "probe", instance.Spec.HealthCheck)
		return "", err
	}
	r.Log.V(1).Info("health check was found, checking if it needs update")
	...
	if !reflect.DeepEqual(newHealthCheckConfig, currentHealthCheck.HealthCheckConfig) {
		...
		r.Log.V(1).Info("health check needs update, updating")
		...
		if err != nil {
			r.Log.Error(err, "unable to update aws health check", "probe", updateHealthCheckInput)
			return "", err
		}
		return *result.HealthCheck.Id, nil
	}
	r.Log.V(1).Info("health does not need update")
	return *currentHealthCheck.Id, nil
}

func (r *GlobalDNSRecordReconciler) createHealthCheck(probe *corev1.Probe, route53Client *route53.Route53, ip string) (string, error) {
	healthCheckConfig, err := r.getAWSHealthCheckConfig(probe, ip)
	if err != nil {
		r.Log.Error(err, "unable to convert probe to aws health check", "probe", probe)
		return "", err
	}
	...
	if err != nil {
		r.Log.Error(err, "unable to cretae AWS", "health check", healthCheckInput)
		return "", err
	}
...
func (r *GlobalDNSRecordReconciler) getAWSHealthCheckConfig(probe *corev1.Probe, ip string) (*route53.HealthCheckConfig, error) {
	if probe == nil || probe.HTTPGet == nil {
		...
		r.Log.Error(err, "healh check must be of http get type")
		return nil, err
	}
...
func (r *GlobalDNSRecordReconciler) ensureRoute53TrafficPolicy(instance *redhatcopv1alpha1.GlobalDNSRecord, route53Client *route53.Route53, endpointMap map[string]EndpointStatus) (string, error) {
	trafficPolicies, err := route53Client.ListTrafficPolicies(&route53.ListTrafficPoliciesInput{})
	if err != nil {
		r.Log.Error(err, "unable to list route53 traffic policies")
		return "", err
	}
	...
	for _, trafficPolicySummary := range trafficPolicies.TrafficPolicySummaries {
		if apis.GetKeyShort(instance) == *trafficPolicySummary.Name {
			r.Log.V(1).Info("found existing", "traffic policy", *trafficPolicySummary.Name)
			...
			if err != nil {
				r.Log.Error(err, "unable to look up", "policy", apis.GetKeyShort(instance))
				return "", err
			}
	...
	if currentTrafficPolicy == nil {
		...
		r.Log.V(1).Info("traffic policy not found creating a new one")
		trafficPolicyID, err := r.createAWSTrafficPolicy(instance, endpointMap, route53Client)
		if err != nil {
			r.Log.Error(err, "unable to create route53 policy")
			return "", err
		}
	...
	r.Log.V(1).Info("traffic policy found checking if it needs updating")
	newPolicyDocument, err := r.getAWSTrafficPolicyDocument(instance, endpointMap, route53Client)
	if err != nil {
		r.Log.Error(err, "unable to create traffic policy document")
		return "", err
	}
	//log.V(1).Info("policy document", "current", *currentTrafficPolicy.Document)
	//log.V(1).Info("policy document", "new", newPolicyDocument)
	// TODO rewrite this one
	same, err := r.IsSameTrafficPolicyDocument(newPolicyDocument, *currentTrafficPolicy.Document)
	//log.V(1).Info("", "isSame", same)
	//log.V(1).Info("", "reflect", reflect.DeepEqual(newPolicyDocument, *currentTrafficPolicy.Document))
	if err != nil {
		r.Log.Error(err, "unable to compare traffic policies")
		return "", err
	}
	if !same {
		// we need to update the health check
		//TODO first delete any existing traffic policy instances
		r.Log.V(1).Info("traffic needs updating, deleting and recreating")
		err = r.deleteTrafficPolicy(currentTrafficPolicy, route53Client)
		if err != nil {
			r.Log.Error(err, "unable to delete route53 traffic policy")
			return "", err
		}
		trafficPolicyID, err := r.createAWSTrafficPolicy(instance, endpointMap, route53Client)
		if err != nil {
	...
	r.Log.V(1).Info("traffic does not need updating")
	return *currentTrafficPolicy.Id, nil
}

func (r *GlobalDNSRecordReconciler) deleteTrafficPolicy(trafficPolicy *route53.TrafficPolicy, route53Client *route53.Route53) error {
	...
	if err != nil {
		r.Log.Error(err, "unable to list traffic policies instances", "by traffic policy id", trafficPolicy.Id)
		return err
	}
	for _, trafficpolicyinstance := range trafficPolicyInstances.TrafficPolicyInstances {
		...
		if err != nil {
			r.Log.Error(err, "unable to delete route53", " traffic policy instance", trafficpolicyinstance.TrafficPolicyId)
			return err
		}
	}
	...
	if err != nil {
		r.Log.Error(err, "unable to unmarshall traffic policy", "document", *trafficPolicy.Document)
		return err
	}
	...
	if err != nil {
		r.Log.Error(err, "unable to delete traffic policy", "id", trafficPolicy)
		return err
	}
	return nil
}

func (r *GlobalDNSRecordReconciler) createAWSTrafficPolicy(instance *redhatcopv1alpha1.GlobalDNSRecord, endpointMap map[string]EndpointStatus, route53Client *route53.Route53) (string, error) {
	document, err := r.getAWSTrafficPolicyDocument(instance, endpointMap, route53Client)
	if err != nil {
		r.Log.Error(err, "unable to create policy document")
		return "", err
	}
	...
	if err != nil {
		r.Log.Error(err, "unable to create", "network policy", trafficPolicy)
		return "", err
	}
	r.Log.V(1).Info("created", "traffic policy id", *result.TrafficPolicy.Id)
	return *result.TrafficPolicy.Id, nil
}

func (r *GlobalDNSRecordReconciler) ensureRoute53DNSRecord(instance *redhatcopv1alpha1.GlobalDNSRecord, globalzone *redhatcopv1alpha1.GlobalDNSZone, route53Client *route53.Route53, trafficPolicyID string) (string, error) {
	...
	if err != nil {
		r.Log.Error(err, "unable to list traffic policies instances", "in zone id", globalzone.Spec.Provider.Route53.ZoneID)
		return "", err
	}
	var currentTrafficPolicyInstance *route53.TrafficPolicyInstance
	for _, trafficPolicyInstance := range trafficPolicyInstances.TrafficPolicyInstances {
		r.Log.V(1).Info("traffic policy instance", "name", *trafficPolicyInstance.Name)
		if *trafficPolicyInstance.Name == instance.Spec.Name+"." {
			r.Log.V(1).Info("found ", "traffic policy instance", instance.Spec.Name)
			currentTrafficPolicyInstanceOutput, err := route53Client.GetTrafficPolicyInstance(&route53.GetTrafficPolicyInstanceInput{
				Id: trafficPolicyInstance.Id,
			})
			if err != nil {
				r.Log.Error(err, "unable to look up", "traffic policy instance", instance.Spec.Name)
				return "", err
			}
	...
	if currentTrafficPolicyInstance == nil {
		//we need to create a new traffic policy
		r.Log.V(1).Info("traffic policy instance was not found, creating a new one...", "traffic policy instance", instance.Spec.Name)
		policyInstanceID, err := r.createAWSTrafficPolicyInstance(instance, globalzone, route53Client, trafficPolicyID)
		if err != nil {
			r.Log.Error(err, "unable to create route53 traffic policy instance")
			return "", err
		}
		return policyInstanceID, nil
	}
	//we need to check if current taffic policy instance is current and if not update it
	r.Log.V(1).Info("traffic policy instance was found, checking if it is still current", "traffic policy instance", instance.Spec.Name)
	if !r.isSameRoute53TrafficPolicyInstance(currentTrafficPolicyInstance, instance, globalzone) {
		r.Log.V(1).Info("found traffic policy not current, deleting it...", "traffic policy instance", instance.Spec.Name)
		// we need to update the health check
		_, err := route53Client.DeleteTrafficPolicyInstance(&route53.DeleteTrafficPolicyInstanceInput{
			Id: currentTrafficPolicyInstance.Id,
		})
		if err != nil {
			r.Log.Error(err, "unable to delete route53", " traffic policy instance", currentTrafficPolicyInstance)
			return "", err
		}
		r.Log.V(1).Info("found traffic policy deleted, creating a new one...", "traffic policy instance", instance.Spec.Name)
		policyInstanceID, err := r.createAWSTrafficPolicyInstance(instance, globalzone, route53Client, trafficPolicyID)
		if err != nil {
			r.Log.Error(err, "unable to create route53 traffic policy instance")
			return "", err
...
func (r *GlobalDNSRecordReconciler) isSameRoute53TrafficPolicyInstance(trafficPolicyInstance *route53.TrafficPolicyInstance, instance *redhatcopv1alpha1.GlobalDNSRecord, globalzone *redhatcopv1alpha1.GlobalDNSZone) bool {
	if *trafficPolicyInstance.HostedZoneId != globalzone.Spec.Provider.Route53.ZoneID {
		r.Log.V(1).Info("zoneid not the same")
		return false
	}
	if *trafficPolicyInstance.Name != instance.Spec.Name+"." {
		r.Log.V(1).Info("name not the same")
		return false
	}
	if *trafficPolicyInstance.TTL != int64(instance.Spec.TTL) {
		r.Log.V(1).Info("ttl not the same")
		return false
	}
	if *trafficPolicyInstance.TrafficPolicyId != instance.Status.ProviderStatus.Route53.PolicyID {
		r.Log.V(1).Info("policyid not the same")
		return false
	}
	return true
}
...
func (r *GlobalDNSRecordReconciler) createAWSTrafficPolicyInstance(instance *redhatcopv1alpha1.GlobalDNSRecord, globalzone *redhatcopv1alpha1.GlobalDNSZone, route53Client *route53.Route53, trafficPolicyID string) (string, error) {
	...
	if err != nil {
		r.Log.Error(err, "unable to create", "traffic policy instance", createTrafficPolicyInstanceInput)
		return "", err
	}
	return *result.TrafficPolicyInstance.Id, nil
}

func (r *GlobalDNSRecordReconciler) IsSameTrafficPolicyDocument(left string, right string) (bool, error) {
	...
	if err != nil {
		r.Log.Error(err, "unable to unmarshal", "traffic policy", left)
		return false, err
	}
...
func (r *GlobalDNSRecordReconciler) getAWSTrafficPolicyDocument(instance *redhatcopv1alpha1.GlobalDNSRecord, endpointMap map[string]EndpointStatus, route53Client *route53.Route53) (string, error) {
	...
	case redhatcopv1alpha1.Multivalue:
		{
			for _, endpoint := range endpointMap {
				...
				if err != nil {
					r.Log.Error(err, "unable to get IPs for", "endpoint", endpoint)
					return "", err
				}
				for _, IP := range IPs {
	...
	default:
		{
			...
			r.Log.Error(err, "illegal routing policy", "policy", instance.Spec.LoadBalancingPolicy)
			return "", err
		}
	}
			...
			for _, endpoint := range endpointMap {
				...
				for _, IP := range IPs {
					...
					if instance.Spec.HealthCheck != nil {
						healthCheckID, err := r.ensureRoute53HealthCheck(instance, route53Client, IP)
						if err != nil {
							r.Log.Error(err, "unable to create healthcheck for", "endpoint", endpoint)
							return "", err
						}
	...
	default:
		{
			...
			r.Log.Error(err, "illegal routing policy")
			return "", err
		}
	...
	if err != nil {
		r.Log.Error(err, "unable to marshal policy document to json", "document", route53TrafficPolicyDocument)
		return "", err
	}
	return string(document), nil
}

func (r *GlobalDNSRecordReconciler) cleanUpRoute53DNSRecord(context context.Context, instance *redhatcopv1alpha1.GlobalDNSRecord, globalzone *redhatcopv1alpha1.GlobalDNSZone) error {
	route53Client, err := tpdroute53.GetRoute53Client(context, globalzone, &r.ReconcilerBase)
	if err != nil {
		r.Log.Error(err, "unable to get route53 client")
		return err
	...
	for _, trafficPolicySummary := range trafficPolicies.TrafficPolicySummaries {
		if apis.GetKeyShort(instance) == *trafficPolicySummary.Name {
			...
			err = r.deleteTrafficPolicy(currentTrafficPolicyOutput.TrafficPolicy, route53Client)
			if err != nil {
				r.Log.Error(err, "unable to delete", "traffic policy", apis.GetKeyShort(instance))
				return err
			}
		}
	}
	...
	if instance.Spec.HealthCheck != nil && instance.Spec.HealthCheck.HTTPGet != nil {
		for _, healthCheck := range healthChecks.HealthChecks {
			if healthCheck.HealthCheckConfig.FullyQualifiedDomainName != nil && *healthCheck.HealthCheckConfig.FullyQualifiedDomainName == instance.Spec.HealthCheck.HTTPGet.Host {
				...
				if err != nil {
					r.Log.Error(err, "unable to delete", "health check for ", *healthCheck.HealthCheckConfig.FullyQualifiedDomainName, "id", healthCheck.Id)
					return err
				}
			}
		}

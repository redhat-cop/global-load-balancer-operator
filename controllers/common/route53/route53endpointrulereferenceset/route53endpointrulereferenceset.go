...
import (
	...
	"github.com/redhat-cop/global-load-balancer-operator/controllers/common/route53"
)

var (
	// helpful to not write everywhere struct{}{}
	keyExists   = struct{}{}
	nonExistent route53.Route53EndpointRuleReference
)
...
type Set struct {
	m map[route53.Route53EndpointRuleReference]struct{}
}

// New creates and initializes a new Set.
func New(ts ...route53.Route53EndpointRuleReference) *Set {
...
func NewWithSize(size int) *Set {
	return &Set{make(map[route53.Route53EndpointRuleReference]struct{}, size)}
}

// Add includes the specified items (one or more) to the Set. The underlying
// Set s is modified. If passed nothing it silently returns.
func (s *Set) Add(items ...route53.Route53EndpointRuleReference) {
...
func (s *Set) Remove(items ...route53.Route53EndpointRuleReference) {
	for _, item := range items {
		delete(s.m, item)
	}
}
...
func (s *Set) Pop() route53.Route53EndpointRuleReference {
	for item := range s.m {
		delete(s.m, item)
		return item
	}
...
// returned.
func (s *Set) Pop2() (route53.Route53EndpointRuleReference, bool) {
	for item := range s.m {
		delete(s.m, item)
		return item, true
	}
...
func (s *Set) Has(items ...route53.Route53EndpointRuleReference) bool {
	has := false
	for _, item := range items {
...
func (s *Set) HasAny(items ...route53.Route53EndpointRuleReference) bool {
	has := false
	for _, item := range items {
		if _, has = s.m[item]; has {
			break
...
func (s *Set) Clear() {
	s.m = make(map[route53.Route53EndpointRuleReference]struct{})
}
...
func (s *Set) IsEqual(t *Set) bool {
	...
	equal := true
	t.Each(func(item route53.Route53EndpointRuleReference) bool {
		_, equal = s.m[item]
...
func (s *Set) Each(f func(item route53.Route53EndpointRuleReference) bool) {
	for item := range s.m {
		if !f(item) {
			break
		}
...
func (s *Set) List() []route53.Route53EndpointRuleReference {
	v := make([]route53.Route53EndpointRuleReference, 0, s.Size())
	for item := range s.m {
		v = append(v, item)
	}
	return v
